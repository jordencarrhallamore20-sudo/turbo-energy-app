...
type ReportMachineRow = {
  ...
  sameTypePercent: number;
  sameTypeAvailable: number;
  sameTypeActive: number;
  departmentPercent: number;
  departmentAvailable: number;
  departmentActive: number;
  departmentTypePercent: number;
  departmentTypeAvailable: number;
  departmentTypeActive: number;
  hoursWorked: number;
  hoursDown: number;
  reason: string;
};
...
export default function DashboardClient({ role, username }: DashboardClientProps) {
  ...
  async function generateReport() {
    ...
    if (!reportFrom && !reportTo) {
      setReportData(
        historyItems.filter((item) => selected.size === 0 || selected.has(item.fleet))
      );
      return;
    }

    if (!reportFrom || !reportTo) {
      alert("Select both From date and To date, or leave both dates blank for current machine report.");
      return;
    }
    ...
    const filtered = ((data as HistoryItem[]) || []).filter(
      (item) => selected.size === 0 || selected.has(item.fleet)
    );
  ...
  function exportReportCsv() {
    const selectedMachineText = reportMachines.length > 0 ? reportMachines.join(", ") : "All machines";

    const summaryRows = [
      ["Turbo Energy Machine Availability Report"],
      ["Generated by", username],
      ["Generated on", reportGeneratedAt || new Date().toLocaleString()],
      ["Date range", `${reportFrom || "Not selected"} to ${reportTo || "Not selected"}`],
      ["Machines", selectedMachineText],
      [],
      ["REPORT SUMMARY"],
      ["Selected machines", reportOverview.total],
      ["Active counted", reportOverview.active],
      ["Available", reportOverview.available],
      ["Down / not available", reportOverview.down],
      ["Major repair excluded", reportOverview.majorExcluded],
      ["Average availability %", formatNumber(reportOverview.percent)],
      ["Hours worked", formatNumber(reportOverview.hoursWorked)],
      ["Hours down", formatNumber(reportOverview.hoursDown)],
      [],
    ];
    ...
    const machineRows = reportMachineRows.map((row) => [
      row.fleet,
      row.machineType,
      row.reportType,
      row.department,
      row.status,
      row.included,
      formatNumber(row.machinePercent),
      formatNumber(row.sameTypePercent),
      `${row.sameTypeAvailable}/${row.sameTypeActive}`,
      formatNumber(row.departmentPercent),
      `${row.departmentAvailable}/${row.departmentActive}`,
      formatNumber(row.departmentTypePercent),
      `${row.departmentTypeAvailable}/${row.departmentTypeActive}`,
      formatNumber(row.hoursWorked),
      formatNumber(row.hoursDown),
      row.reason,
    ]);

    const groupHeaders = ["Group", "Total", "Active", "Available", "Down", "Major excluded", "Availability %", "Hours worked", "Hours down"];

    const groupRows = (rows: ReportGroupRow[]) =>
      rows.map((row) => [
        row.name,
        row.total,
        row.active,
    ...
    const historyHeaders = ["Date", "Fleet", "Action", "Field", "Change", "Notes", "Actor"];
    const historyRows = reportData.map((item) => [
    ...
    const csvRows = [
      ...
      groupHeaders,
      ...groupRows(reportTypeSummary),
      [],
      ["DEPARTMENT AVAILABILITY"],
      groupHeaders,
      ...groupRows(reportDepartmentSummary),
      [],
      ["DEPARTMENT + MACHINE TYPE AVAILABILITY"],
      groupHeaders,
      ...groupRows(reportDepartmentTypeSummary),
      [],
      ["HISTORY RECORDS"],
      historyHeaders,
      ...(historyRows.length > 0 ? historyRows : [["No history records in this report"]]),
    ];
    ...
    link.href = url;
    link.download = "machine-availability-report.csv";
    link.click();
  ...
  const selectedReportMachines = useMemo(() => {
  ...
  const reportOverview = useMemo(() => buildReportGroup("Selected Machines", selectedReportMachines), [selectedReportMachines]);

  const reportTypeSummary = useMemo(() => {
    return buildReportGroupRows(selectedReportMachines, (machine) => normalizeTypeLabel(machine.type || inferTypeFromFleet(machine.fleet)));
  }, [selectedReportMachines]);

  const reportDepartmentSummary = useMemo(() => {
    return buildReportGroupRows(selectedReportMachines, (machine) => machine.department || "Unassigned");
  }, [selectedReportMachines]);

  const reportDepartmentTypeSummary = useMemo(() => {
    return buildReportGroupRows(
      selectedReportMachines,
      (machine) => `${machine.department || "Unassigned"} - ${normalizeTypeLabel(machine.type || inferTypeFromFleet(machine.fleet))}`
    );
  }, [selectedReportMachines]);

  const reportMachineRows = useMemo<ReportMachineRow[]>(() => {
    return selectedReportMachines
      .slice()
      .sort((a, b) => a.fleet.localeCompare(b.fleet))
      .map((machine) => {
        const reportType = normalizeTypeLabel(machine.type || inferTypeFromFleet(machine.fleet));
        const department = machine.department || "Unassigned";
        const sameTypeMachines = machines.filter(
          (item) => normalizeTypeLabel(item.type || inferTypeFromFleet(item.fleet)) === reportType
        );
        ...
        const departmentTypeMachines = machines.filter(
          (item) =>
            ...
            normalizeTypeLabel(item.type || inferTypeFromFleet(item.fleet)) === reportType
        );

        const sameType = buildReportGroup(reportType, sameTypeMachines);
        const departmentGroup = buildReportGroup(department, departmentMachines);
        const departmentType = buildReportGroup(`${department} - ${reportType}`, departmentTypeMachines);
        ...
        return {
          ...
          reportType,
          department,
          status: machine.majorRepair ? "Major Repair" : machine.status,
          included: machine.majorRepair ? "No" : "Yes",
          machinePercent: machine.majorRepair ? 0 : Number(machine.availability) || 0,
          sameTypePercent: sameType.percent,
          sameTypeAvailable: sameType.available,
          sameTypeActive: sameType.active,
          departmentPercent: departmentGroup.percent,
          departmentAvailable: departmentGroup.available,
          departmentActive: departmentGroup.active,
          departmentTypePercent: departmentType.percent,
          departmentTypeAvailable: departmentType.available,
          departmentTypeActive: departmentType.active,
          hoursWorked: Number(formatNumber(machine.hoursWorked)) || 0,
          hoursDown: Number(formatNumber(machine.hoursDown)) || 0,
          reason: machine.downtimeReason || machine.repairReason || "-",
        };
      });
  }, [machines, selectedReportMachines]);
  ...
  return (
    <div className="page">
      <div className="shell">
        ...
        <section className="panel reportPanel reportPanelStandalone" id="report-generator">
          ...
          <div className="reportVisualGrid">
            <div className="reportVisualCard">
              <div className="sectionTitleRow reportSectionRow">
                <h2>Availability by Machine Type</h2>
                <span>% Available</span>
              </div>

              <div className="chartPanel reportChartPanel">
                ...
                <div className="barsArea">
                  {reportTypeSummary.length === 0 ? (
                    <div className="mutedCard reportEmptyState">No machine type data for this report.</div>
                  ) : (
                    reportTypeSummary.map((item, index) => (
                      <div key={`report-type-${item.name}`} className="barGroup">
                        <div className="barValue">{Math.round(item.percent)}%</div>
                        <div
                          className={`bar ${index % 4 === 3 ? "highlightBar" : ""}`}
                          style={{ height: `${Math.max(item.percent * 1.7, 18)}px` }}
                        />
              ...
              <p className="footNote">Major repair units are excluded from the report machine type availability graph.</p>
            </div>

            <div className="reportVisualCard">
              <div className="sectionHeading reportVisualHeading">
                <h2>Department Availability</h2>
                <p>Percentages for the selected report machines.</p>
              </div>

              <div className="departmentGrid reportDepartmentGrid">
                {reportDepartmentSummary.length === 0 ? (
                  <div className="mutedCard reportEmptyState">No department data for this report.</div>
                ) : (
                  reportDepartmentSummary.map((item) => (
                    <div key={`report-department-${item.name}`} className="departmentCard reportDepartmentCard">
                      <div className="departmentHead">
                        <h3>{item.name}</h3>
                        <span>{Math.round(item.percent)}%</span>
                      </div>
                      <div className="departmentBarTrack">
                        <div className="departmentBarFill" style={{ width: `${Math.max(0, Math.min(100, item.percent))}%` }} />
                      </div>
          ...
          <div className="reportSectionSubTitle">Department + Machine Type Availability</div>
          <div className="reportDepartmentTypeCards">
            {reportDepartmentTypeSummary.length === 0 ? (
              <div className="mutedCard reportEmptyState">No department + machine type data for this report.</div>
            ) : (
              reportDepartmentTypeSummary.map((item) => (
                <div key={`report-dept-type-${item.name}`} className="departmentTypeCard reportDepartmentTypeCard">
                  <div className="departmentHead">
                    <h3>{item.name}</h3>
                    <span>{Math.round(item.percent)}%</span>
                  </div>
            ...
            <table>
              <thead>
                <tr>
                  ...
                  <th>Machine %</th>
                  <th>Same Type %</th>
                  <th>Type Avail/Active</th>
                  <th>Department %</th>
                  <th>Dept Avail/Active</th>
                  <th>Dept + Type %</th>
                  <th>Reason</th>
                </tr>
              </thead>
              <tbody>
                {reportMachineRows.length === 0 ? (
                  <tr>
                    <td colSpan={12}>No machines selected for report.</td>
                  </tr>
                ) : (
                  reportMachineRows.map((row) => (
                    <tr key={`report-machine-${row.fleet}`}>
                      <td>{row.fleet}</td>
                      <td>{row.machineType}</td>
                      <td>{row.reportType}</td>
                      <td>{row.department}</td>
                      <td>{row.status}</td>
                      <td>{row.included === "No" ? "Excluded" : `${formatNumber(row.machinePercent)}%`}</td>
                      <td>{formatNumber(row.sameTypePercent)}%</td>
                      <td>{row.sameTypeAvailable}/{row.sameTypeActive}</td>
                      <td>{formatNumber(row.departmentPercent)}%</td>
                      <td>{row.departmentAvailable}/{row.departmentActive}</td>
                      <td>{formatNumber(row.departmentTypePercent)}%</td>
                      <td>{row.reason}</td>
                    </tr>
                  ))
                )}
          ...
          <div className="tableWrap reportTableWrap">
            <table>
              ...
              <tbody>
                {reportData.length === 0 ? (
                  <tr>
                    <td colSpan={7}>No history records in this report. Machine availability summary is still shown above.</td>
                  </tr>
                ) : (
                  reportData.map((item) => (
                    <tr key={item.id}>
      ...
      <style jsx>{`
        ...
        .reportDepartmentGrid {
          grid-template-columns: repeat(2, minmax(0, 1fr));
        }

        .reportDepartmentCard, .reportDepartmentTypeCard {
          min-width: 0;
        }

        .reportDepartmentTypeCards {
          display: grid;
          grid-template-columns: repeat(4, minmax(0, 1fr));
          gap: 12px;
          margin-top: 8px;
        ...
        .reportGroupTable table { min-width: 760px; }
        ...
        @media (max-width: 860px) {
          ...
          .kpiGrid, .departmentGrid, .detailGrid, .adminGrid, .reportSummaryGrid, .reportVisualGrid, .reportDepartmentGrid, .reportDepartmentTypeCards { grid-template-columns: 1fr; }
        ...
        @media print {
          ...
          .reportSummaryGrid { grid-template-columns: repeat(2, minmax(0, 1fr)); }
          .reportVisualGrid, .reportDepartmentTypeCards { grid-template-columns: 1fr; }
...
function ReportGroupTable({ rows }: { rows: ReportGroupRow[] }) {
  return (
    <div className="tableWrap reportTableWrap reportGroupTable">
      <table>
        <thead>
          <tr>
            <th>Group</th>
        ...
        <tbody>
          {rows.length === 0 ? (
            <tr>
              <td colSpan={9}>No data for this report section.</td>
            </tr>
          ) : (
            rows.map((row) => (
              <tr key={row.name}>
                ...
                <td>{row.available}</td>
                <td>{row.down}</td>
                <td>{row.majorExcluded}</td>
                <td>{formatNumber(row.percent)}%</td>
                <td>{formatNumber(row.hoursWorked)}</td>
                <td>{formatNumber(row.hoursDown)}</td>
              </tr>
            ))
          )}
        </tbody>
...
function isAvailableForReport(machine: Machine) {
  return !machine.majorRepair && machine.status.toLowerCase().includes("avail");
}

function machineAvailabilityForReport(machine: Machine) {
  if (machine.majorRepair) return 0;
  const value = Number(machine.availability) || 0;
  return Math.max(0, Math.min(100, value));
}

function buildReportGroup(name: string, items: Machine[]): ReportGroupRow {
  const active = items.filter((machine) => !machine.majorRepair);
  const available = active.filter(isAvailableForReport).length;
  const hoursWorked = active.reduce((sum, machine) => sum + (Number(formatNumber(machine.hoursWorked)) || 0), 0);
  const hoursDown = active.reduce((sum, machine) => sum + (Number(formatNumber(machine.hoursDown)) || 0), 0);
  const percent =
    active.length === 0
      ? 0
      : active.reduce((sum, machine) => sum + machineAvailabilityForReport(machine), 0) / active.length;
  ...
  return {
    ...
    active: active.length,
    available,
    down: active.length - available,
    majorExcluded: items.length - active.length,
    percent,
    hoursWorked,
    hoursDown,
  };
...
function buildReportGroupRows(items: Machine[], getName: (machine: Machine) => string): ReportGroupRow[] {
  const groups = new Map<string, Machine[]>();

  items.forEach((machine) => {
    const name = getName(machine) || "Unassigned";
  ...
  return Array.from(groups.entries())
    .map(([name, groupItems]) => buildReportGroup(name, groupItems))
